  ...
  const loadApplicationData = async () => {
    ...
    try {
      ...
      const [logsRes, uptimeRes] = await Promise.all([
        api<{ ok: boolean; logs: StatusLog[] }>(
          `/api/apps/${application.id}/logs?limit=1`
        ),
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            ...
            <p className="font-semibold text-lg">{uptime.toFixed(1)}%</p>
          </div>
          <div>
            <p className="text-muted-foreground">Last Check</p>
            <div className="flex items-center space-x-1">
              <Clock className="w-3 h-3" />
              <p className="font-medium text-xs truncate">{lastChecked}</p>
            </div>
          </div>
        </div>
