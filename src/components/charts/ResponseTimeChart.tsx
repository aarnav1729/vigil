...
const ResponseTimeChart = ({
  ...
  const chartData = React.useMemo(() => {
    const validLogs = logs
      .filter(
        (log) =>
          ...
          Number.isFinite(Number(log.timestamp))
      )
    ...
    return validLogs.map((log, index) => ({
      time: format(
        ...
        showDetails ? "MMM dd HH:mm" : "HH:mm"
      ),
  ...
  const avgResponseTime = React.useMemo(() => {
    ...
    return (
      chartData.reduce((sum, item) => sum + item.responseTime, 0) /
      chartData.length
    );
  }, [chartData]);
  ...
  if (chartData.length === 0) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>
            Response Time {showDetails ? "History" : "(Last 24 checks)"}
          </CardTitle>
        </CardHeader>
        <CardContent>
  ...
  return (
    <Card>
      ...
      <CardContent>
        <ChartContainer
          config={chartConfig}
          className={showDetails ? "h-[400px]" : "h-[300px]"}
        >
