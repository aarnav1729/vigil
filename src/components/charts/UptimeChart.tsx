...
import { calculateUptime } from '@/lib/database';
...
const UptimeChart = ({ applicationId }: UptimeChartProps) => {
  ...
  const loadUptimeData = async () => {
    try {
      ...
      for (let i = 29; i >= 0; i--) {
        ...
        const uptime = await calculateUptime(applicationId, 24); // 24h uptime for each day
  ...
  const avgUptime = React.useMemo(() => {
    ...
    return chartData.reduce((sum, item) => sum + item.uptime, 0) / chartData.length;
  }, [chartData]);
          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
              ...
              <Line
                ...
                activeDot={{ r: 6, stroke: 'var(--color-uptime)', strokeWidth: 2 }}
              />
